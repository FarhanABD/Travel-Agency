...
class AdminWelcomeItemController extends Controller
{
    ...
    $oldPhotoPath = public_path('uploads/welcome/' . $obj->photo);

    if (!empty($obj->photo) && file_exists($oldPhotoPath) && is_file($oldPhotoPath)) {
        unlink($oldPhotoPath);
    }

    // Upload new photo
    $final_name = 'welcome_item_' . time() . '.' . $request->photo->extension();
    $request->photo->move(public_path('uploads/welcome'), $final_name);
